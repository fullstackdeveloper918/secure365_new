...
import SingleProKeyAppleTech from "./SingleProKeyAppleTech"
...
const AppleTech = () => {
  ...
  useEffect(() => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        if (titleRef.current) {
          const id = scrambleText(titleRef.current, "Apple Tech Specs", 1.5);
          if (id) intervalsRef.current.push(id);
        }
        if (spanRef.current) {
          const id = scrambleText(
            spanRef.current,
            "In-depth Apple Device Specifications at Your Fingertips",
            2
          );
          if (id) intervalsRef.current.push(id);
        }
